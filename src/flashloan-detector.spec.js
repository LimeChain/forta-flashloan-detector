...
// Mock the initIronBankMarkets method because
// it calls a JsonRpc provider
jest.mock('./detectors/iron-bank-detector', () => ({
  ...jest.requireActual('./detectors/iron-bank-detector'),
  initIronBankMarkets: () => {},
}));
  ...
  describe('init', () => {
    it('should support all protocols by default', () => {
      ...
      expect(flashloanDetector.protocols).toEqual({
        aave: true, dydx: true, euler: true, ironBank: true, maker: true,
      });
    });

    it('should support only the provided protocols', () => {
      flashloanDetector = new FlashloanDetector(['aave', 'maker']);

      expect(flashloanDetector.protocols).toEqual({
        aave: true, maker: true,
      });
    });
